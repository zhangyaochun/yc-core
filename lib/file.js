...
//hasBOM
file.hasBOM = function (buffer) {
    //The UTF8 BOM [0xEF,0xBB,0xBF]
    return (
        buffer[0] === 0xEF && 
        buffer[1] === 0xBB && 
        buffer[2] === 0xBF
...
//noBOM
file.noBOM = function (buffer) {
    //check if has BOM
    if (file.hasBOM(buffer)) {
...
//isGzip
file.isGzip = function (buffer) {
    //take a look at [gzip-js]
    return (
        buffer[0] === 0x1F && 
        buffer[1] === 0x8B && 
        buffer[2] === 0x08
